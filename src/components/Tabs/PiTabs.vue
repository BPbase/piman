<template>
  <div class="pi-tabs">
    <ul role="tablist" class="pi-tabs-list">
      <li
        role="presentation"
        v-for="(tab, index) in tabs"
        :key="index"
        :class="['pi-tabs__item', tab.name === modelValue ? 'pi-tabs__item--selected' : '']"
      >
        <pi-button
          role="tab"
        ...
        >
          ...
          <span v-if="tab.badge" class="pi-badge" :aria-label="t('badge.pre') + tab.badge">{{
            tab.badge
          }}</span>
        </pi-button>
      </li>
...
const handleClickPrevTab = () => {
  ...
  ;(document as any)
    .getElementById('tab-' + toKebabCase(tabs.value[currentIndex.value]['name']))
    .focus()
